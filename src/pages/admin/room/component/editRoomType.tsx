...
const EditRoomType: React.FC<EditRoomProps> = ({ onClose, roomTypeId }) => {
	...
	const validateField = (name: string, value: string) => {
		if (name !== 'description' && name !== 'newPrice' && (!value || value.trim() === '')) {
			setErrors((prev) => ({
				...prev,
		...
		if (name === 'images') {
			if (images.length === 0) {
				setErrors((prev) => ({ ...prev, [name]: 'Vui lòng tải lên ít nhất 1 hình ảnh' }));
				return false;
			}
		}
	...
	const removeImage = (index: number) => {
		...
		const imageId = values?.images?.find((image) => image.link === images[index])?.id;
					...
					<form className='form-admin-modal position-relative'>
						<div className='row flex-wrap'>
							...
							<div className='col-12 col-sm-12 col-md-12 col-lg-6 col-xl-6 col-xxl-6 form-group mt-3 px-2'>
								...
								/>
								{errors.newPrice && <div className='err-text'>{errors.newPrice}</div>}
							</div>
