...
const SendNotification: React.FC<SendNotificationProps> = ({ notificationId, onClose }) => {
    const [selectedRole, setSelectedRole] = useState<string | null>(null);
    const [availableRoles, setAvailableRoles] = useState<string[]>(['customer', 'admin', 'staff', 'owner']);
    ...
    useEffect(() => {
        ...
        if (token) {
            ...
            const filteredRoles = availableRoles.filter(role => role.toLowerCase() !== userRole);
    ...
    const onSubmit = async (data: SendNoti) => {
        ...
        try {
            const response = await postSendNotiApi(selectedRole, notificationId, data);
                ...
                <div className="modal-content-notifi-send position-relative">
                    ...
                    <form className='form-send-notifi' onSubmit={handleSubmit(onSubmit)}>
                        ...
                        <div className='d-flex justify-content-between mt-4'>
                            <button type="button" className='btn-send-notifi btn-send-notifi-cancel' onClick={onClose}>Trở về</button>
                            <button type="submit" className='btn-send-notifi btn-send-notifi-send'>Gửi</button>
                        </div>
                    </form>
                </div>
            </div>
