...
const RegisterOwner_1 = ({ onNext }: Props) => {
    ...
    const { control, handleSubmit, formState: { errors } } = useForm<Motel>({
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)}>
                    ...
                    <div key={field.id} className="mb-3 row">
                        <div className="col-3">
                            <InputField
                                ...
                                type="text"
                                errors={errors}
                                classname={`form-control ${errors.services?.[index]?.name ? "is-invalid" : ""}`}
                            />
                        </div>
                        <div className="col-3">
                            <InputField
                                ...
                                type="text"
                                errors={errors}
                                classname={`form-control ${errors.services?.[index]?.price ? "is-invalid" : ""}`}
                            />
                        </div>
                        <div className="col-5">
                            <InputField
                                ...
                                type="text"
                                errors={errors}
                                classname={`form-control ${errors.services?.[index]?.description ? "is-invalid" : ""}`}
                            />
                        </div>
            ...
            <div className="d-flex justify-content-end">
                <button className="btn btn-danger" type="submit">Tiếp tục</button>
            </div>
        </form>
    );
};
