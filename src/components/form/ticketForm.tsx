...
const TicketForm = forwardRef(({ onSubmit, className, buttonNone, motel }: Props, ref) => {
    ...
    const { control, handleSubmit, formState: { errors }, reset } = useForm<FormCreate>({
    ...
    return (
        <form className="ticket-form" onSubmit={handleSubmit((data) => {
            onSubmit(data, reset);
            setImages([]);
        })}>
            <div className="row">
